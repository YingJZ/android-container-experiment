...
**结构：**
1. 问题陈述 — 引用实验报告的精确错误信息和已尝试选项
2. 6 个可行方案（A-F），每个包含可行性评级、实施步骤、优劣势、工时估算
3. CRIU 关键选项参考表 — 所有挂载/cgroup/文件系统相关选项
4. CRIU 近期发展 — Mount-v2、K8s 工作组、插件系统局限
5. 对比矩阵 — 6 方案横向比较
6. 推荐执行计划 — 分优先级、有时间线
7. 重要限制说明 — 明确 L1（挂载）和 L2（Binder 状态）是两个独立问题

**核心结论：**
- 推荐方案 A（LXC + CRIU），可行性 8/10，ReDroid 官方支持 LXC 部署，LXC 有成熟 CRIU 集成
- 备选方案 B（Podman + CRIU），可行性 7/10，API 兼容 Docker
- 兜底方案 F（混合方案），绕过 CRIU 直接用 Docker commit + 应用状态序列化
- 当前 Docker 环境下的 CRIU 成功概率较低（方案 C，5/10）
